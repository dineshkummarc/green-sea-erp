<div class="pageContent">
	<form action="<?php echo $this->createUrl("user/adzdUser") ?>" method="post" onsubmit="return validateCallback(this, dialogAjaxDone);">
	    <div class="pageFormContent" layoutH="60">
	        ...
	        <div class="unit">
	            <label>手机号：</label><input type="text" name="Form[mobile_phone]" class="required" value="<?php echo !empty($user->mobile_phone) ? $user->mobile_phone : ''; ?>"/>
	        </div>
	        <div class="unit">
	            <label>密码：</label><input type="password" name="Form[password]" class="required" value="" value="<?php echo !empty($user->password) ? $user->password : ''; ?>"/>
	        </div>
	        <div class="unit">
	            <label>密码验证：</label><input id="repassword" type="password" class="required" />
	        </div>
	        <div class="unit">
	            <label>邮箱地址：</label><input type="text" name="Form[email]" class="required" value="<?php echo !empty($user->email) ? $user->email : ''; ?>" />
	        </div>
	        <div class="unit">
	            <label>所在地区：</label>
