<div class="pageContent" width="100%" layoutH="27">
    <table class="list" id="list" width="1200px">
    	<thead>
	        <tr>
	            <th width="30"></th>
        ...
        <tbody>
	        ...
	        <tr>
	            ...
	            <td><?php echo substr(strval($orderTrack->Order->user_id + 1000),1,3)?></td><!-- 客户编号 -->
	            <td><?php echo $orderTrack->Order->user_name?></td><!-- 客户名称 -->
	            <td><?php echo $orderTrack->photographer_id !=0 ? "<span style='color:red'>✔</span>" : "";?></td><!-- 确认拍摄 -->
	            <td><?php
	            	$storage = $orderTrack->getStorage();
	            	echo empty($storage)?'无仓储':date('Y-m-d',$storage['in_time']);
	            ?></td><!-- 时间 -->
	            <td><?php
	            	if (empty($storage))
	            		echo '无仓储';
	            	else{
	            		$count = $orderTrack->getStorageGoodsCount($storage['id']);
	            		echo empty($count)?'0':$count;
	            	}
	            ?></td><!-- 数量 -->
	            <td><?php echo $photographer_id != 0 ? date('Y-m-d',$shoot_begin_time):''?></td><!-- 拍摄时间 -->
	            <td></td><!-- 模特 -->
	            <td><?php echo $photographer_id != 0 ? "<span style='color:red'>✔</span>" : "";?></td><!-- 拍摄中 -->
	            <td><?php echo $photographer_id_2 != 0 ? "<span style='color:red'>✔</span>" : "";?></td><!-- 已拍摄 -->
	            <td><?php echo $retouch_id != 0 ? "<span style='color:red'>✔</span>" : "";?></td><!-- 修中 -->
	            <td><?php echo $retouch_id_2 != 0 ? "<span style='color:red'>✔</span>" : "";?></td><!-- 修完 -->
	        ...
	        <tr>
	            ...
	            <td></td><!-- 模特 -->
	            <td>
	            	<?php echo "<span style='font-weight:bold'>".Admin::getAdminName($photographer_id).'</span>';?><br/>
	            	<?php echo $shoot_begin_time == 0 ? '' : date('Y-m-d H:s',$shoot_begin_time)?>
	            </td><!-- 拍摄中 -->
	            <td>
	            	<?php echo "<span style='font-weight:bold'>".Admin::getAdminName($photographer_id_2).'</span>';?><br/>
	            	<?php echo $shoot_end_time == 0 ? '' : date('Y-m-d H:s',$shoot_end_time)?>
	            </td><!-- 已拍摄 -->
	            <td>
	            	<?php echo "<span style='font-weight:bold'>".Admin::getAdminName($retouch_id).'</span>';?><br/>
	            	<?php echo $retouch_begin_time == 0 ? '' : date('Y-m-d H:s',$retouch_begin_time)?>
	            </td><!-- 修中 -->
	            <td>
	            	<?php echo "<span style='font-weight:bold'>".Admin::getAdminName($retouch_id_2).'</span>';?><br/>
	            	<?php echo $retouch_end_time == 0 ? '' : date('Y-m-d H:s',$retouch_end_time)?>
	            </td><!-- 修完 -->
	            <td></td><!-- 是否付款 -->
	            <td></td><!-- 已修图 -->
	            <td></td><!-- 已交图 -->
